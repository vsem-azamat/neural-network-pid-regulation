...
def compare_controllers(trolley, lstm_regulator, pid, rbf_model, X_normalizer, y_normalizer, setpoint, steps, dt, warm_up_steps):
    ...
    # Plot positions
    axs[0].plot(lstm_results[0], lstm_results[1], label='LSTM PID', color='#1f77b4', linewidth=2)
    axs[0].plot(default_results[0], default_results[1], label='Default PID', color='#ff7f0e', linewidth=2)
    axs[0].axhline(y=setpoint, color='r', linestyle='--', label='Setpoint', linewidth=2)
    axs[0].axvline(x=warm_up_steps*dt.item(), color='g', linestyle='--', label='LSTM Start', linewidth=2)
    ...
    axs[1].plot(lstm_results[0], lstm_results[2], label='LSTM Control', color='#2ca02c', linewidth=2)
    axs[1].plot(default_results[0], default_results[2], label='Default Control', color='#d62728', linewidth=2)
    axs[1].axvline(x=warm_up_steps*dt.item(), color='g', linestyle='--', label='LSTM Start', linewidth=2)
    ...
    axs[2].plot(lstm_results[0], lstm_results[3], label='LSTM Kp', color='#1f77b4', linewidth=2)
    axs[2].plot(lstm_results[0], lstm_results[4], label='LSTM Ki', color='#ff7f0e', linewidth=2)
    axs[2].plot(lstm_results[0], lstm_results[5], label='LSTM Kd', color='#2ca02c', linewidth=2)
    axs[2].plot(default_results[0], default_results[3], label='Default Kp', color='#1f77b4', linestyle='--', linewidth=2)
    axs[2].plot(default_results[0], default_results[4], label='Default Ki', color='#ff7f0e', linestyle='--', linewidth=2)
    axs[2].plot(default_results[0], default_results[5], label='Default Kd', color='#2ca02c', linestyle='--', linewidth=2)
    axs[2].axvline(x=warm_up_steps*dt.item(), color='g', linestyle='--', label='LSTM Start', linewidth=2)
    ...
    def calculate_metrics(positions, setpoint, start_index):
        ...
        settling_time = next((i for i, e in enumerate(error) if e < 0.05 * abs(setpoint)), len(error)) * dt.item()
        overshoot = max(0, max(positions[start_index:]) - setpoint) / abs(setpoint) * 100
...
if __name__ == "__main__":
    ...
    compare_controllers(trolley, lstm_model, pid, rbf_model, X_normalizer, y_normalizer, compare_setpoint, steps, dt, warm_up_steps)