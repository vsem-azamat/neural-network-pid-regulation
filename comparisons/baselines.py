...
def optimise_fixed_gains(
    ...
    ``start_from`` makes those gains the initial incumbent, so the result is
    never worse than them. The headroom oracle needs that guarantee: a
    per-episode search that restarts from the middle of the box can stall in
    a worse basin than the global constant it is being compared against, and
    an "oracle" that loses to the thing it bounds makes the measured headroom
    meaningless.
    ...
    if start_from is None:
        ...
        centre = np.clip(np.array(start_from, dtype=float), 0.0, ceiling_array)
    best_gains = tuple(centre)
