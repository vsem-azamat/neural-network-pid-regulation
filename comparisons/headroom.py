...
Run it *before* concluding anything from a scheduler that fails to beat
constant gains: a small headroom means the result describes the problem, not
the method. No network is involved, only the plant and the objective.

    one global constant                      what a fixed PID gets
      vs best constant for each episode      between-episode headroom:
                                             knowing which plant you are on
      vs best gain table for each episode    within-episode headroom:
                                             scheduling on the operating point

Both oracles are searched on the episode they are scored on, so they are upper
bounds, not targets. Each search starts from the previous level's answer, so an
oracle can never score worse than what it bounds.

On a linear plant, within one episode, the optimal fixed-structure controller
is constant, so the within-episode gap there comes only from actuator
saturation and the heater's inability to cool.
"""
...
def main(system_name: str, seed: int, runs: int, iterations: int,
    ...
    for index, episode in enumerate(episodes):
        gains, score = baselines.optimise_fixed_gains(
            ...
            iterations=iterations,
            seed=seed + index,
            # Start at the global constant, so the oracle cannot lose to it.
            start_from=global_gains,
        )
