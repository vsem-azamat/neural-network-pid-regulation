...
def test_imc_with_dead_time_uses_the_standard_rule():
    """IMC-PID for a FOPDT plant: Kp = (2T + L) / (K (2 lambda + L)).

    The dead time was missing from the denominator, which inflated all three
    gains by (2 lambda + L) / (2 lambda) - about 1.6x on the radiative plant.
    """
    model = FOPDT(K=0.5, L=2.0, T=30.0)
    lam = 0.8 * model.L
    Kp, Ki, Kd = tuning.pid_imc(model)
    assert Kp == pytest.approx((2 * model.T + model.L) / (model.K * (2 * lam + model.L)))
    assert Ki == pytest.approx(Kp / (model.T + model.L / 2))
    assert Kd == pytest.approx(Kp * model.T * model.L / (2 * model.T + model.L))
