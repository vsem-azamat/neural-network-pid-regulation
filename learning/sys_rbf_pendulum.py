...
def generate_training_data(pendulum_cart, num_samples=2000):
    ...
    y = torch.zeros((num_samples, 4))  # [next_x, next_x_dot, next_theta, next_theta_dot]
    ...
    for i in range(num_samples):
        ...
        control_input = torch.rand(1) * 20 - 10  # Random control input between -10 and 10
        ...
        X[i] = torch.tensor([x.item(), x_dot.item(), theta.item(), theta_dot.item(), control_input.item()])
        y[i] = next_state

    return X, y

def train_rbf_model(model, pendulum_cart, num_epochs=500, batch_size=32, learning_rate=0.001):
...
def compare_predictions(model, pendulum_cart, X_normalizer, y_normalizer, num_steps=200):
    initial_state = torch.tensor([0.5, 0.0, 0.1, 0.0])  # Initial state: x, x_dot, theta, theta_dot
    control_inputs = torch.linspace(-5, 5, num_steps)

    rbf_states = []
    actual_states = []

    pendulum_cart.x, pendulum_cart.x_dot, pendulum_cart.theta, pendulum_cart.theta_dot = initial_state
    ...
    for control in control_inputs:
        ...
        with torch.no_grad():
            ...
            rbf_next_state = y_normalizer.denormalize(rbf_next_state_normalized).squeeze(0)
        ...
        pendulum_cart.x, pendulum_cart.x_dot, pendulum_cart.theta, pendulum_cart.theta_dot = actual_next_state
...
def plot_comparison(control_inputs, rbf_states, actual_states):
    ...
    for i in range(4):
        row = i // 2
        col = i % 2
        axs[row, col].plot(control_inputs, rbf_states[:, i], label='RBF Model', marker='o', linestyle='-', markersize=3)
        axs[row, col].plot(control_inputs, actual_states[:, i], label='Actual System', marker='x', linestyle='-', markersize=3)
...
if __name__ == "__main__":
    ...
    losses, X_normalizer, y_normalizer = train_rbf_model(rbf_model, pendulum_cart, num_epochs=500, learning_rate=0.001)
    ...
    # Compare predictions
    control_inputs, rbf_states, actual_states = compare_predictions(rbf_model, pendulum_cart, X_normalizer, y_normalizer)
    ...
    for i, state_label in enumerate(['Cart Position', 'Cart Velocity', 'Pendulum Angle', 'Pendulum Angular Velocity']):
